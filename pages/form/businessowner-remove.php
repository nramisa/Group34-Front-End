...
if (isset($_POST['RemoveOwner'])) {
    $remove_ID = $_POST['RemoveOwnerID'];

    $sql_remove_owner = "DELETE FROM `business_owner` WHERE `OwnerID`='$remove_ID'";
    $result_remove_owner = $conn->query($sql_remove_owner);

    $sql_remove_ownedbusiness = "DELETE FROM `ownedbusiness` WHERE `OwnerID`='$remove_ID'";
    $result_remove_ownedbusiness = $conn->query($sql_remove_ownedbusiness);

    if ($result_remove_owner === TRUE && $result_remove_ownedbusiness === TRUE) {
...
<meta charset="utf-8">
  ...
  <title>Administrator | Remove Business Owner</title>
      ...
      <div class="col-md-6">
        <div class="card card-danger">
            <div class="card-header">
                <h3 class="card-title">Remove Owner</h3>
            ...
            <div class="card-body">
                <form action="" method="POST">
                    <div class="form-group">
                        <label for="inputRemoveOwnerID">Business Owner ID</label>
                        <input type="text" name="RemoveOwnerID" id="inputRemoveOwnerID" class="form-control">
                    </div>
                    <div class="form-group">
                        <input type="submit" name="RemoveOwner" value="Remove" class="btn btn-danger">
                    </div>
                </form>
            </div>
        </div>
